"""
Usage:
    python training_length.py [NUM_TRIALS] [LEN_TRAINING_LENGTH_RANGE]
For example:
    python nodes.py 100 5
represents simulating the number of training data points at 20000, 40000, 60000, 
80000, 100000. For each simulation, we perform 100 trials to smooth out the randomness.
"""
...
NUM_TRAINING_LENGTH_VALS = int(sys.argv[2])
TRAINING_LENGTH_RANGE = [20000 * (n + 1) for n in range(NUM_TRAINING_LENGTH_VALS)]
...
def trial(trial_id, training_length, training_data_ls, test_data_ls):    
    trial_dir = "results/training_lengths/{}-{}-{}-{}/{}/{}/".format(today, NUM_TRIALS, TRAINING_LENGTH_RANGE[0], TRAINING_LENGTH_RANGE[-1], training_length, trial_id)
    make_dir(trial_dir)
    ...
    print("\t\tWriting results for training length: {}, Trial: #{} / {}, Loss: {}...".format(training_length, trial_id, NUM_TRIALS, losses[1]), end="")
    np.savez_compressed(
        trial_dir + "results",
        W_r=W_r,
        W_in=W_in,
...
if __name__ == '__main__':
    ...
    dir = "results/training_lengths/{}-{}-{}-{}/".format(today, NUM_TRIALS, TRAINING_LENGTH_RANGE[0], TRAINING_LENGTH_RANGE[-1])
    make_dir(dir)
    
    print("Welcome.\nToday's date is: {}.\nIt is a good day for a simulation.\n".format(today))
    print("Below are the simulation parameters.")
    print("Training lengths tested: {}".format(str(TRAINING_LENGTH_RANGE)[1:-1]))
    print("Number of trials per training length value: {}".format(NUM_TRIALS))

    start = datetime.now()

    print("Beginning simulation...")

    # perform NUM_TRIALS trials for each training length value
    print("\tPerforming trials...")
    for training_length in TRAINING_LENGTH_RANGE:
        # generate datasets
        simulation_parameters["WASHOUT"] = training_length // 2
        simulation_parameters["STEPS"] = simulation_parameters["WASHOUT"] + training_length
        ...
        paramgrid = list(itertools.product(trial_range, [training_length], [training_data_ls], [test_data_ls]))
    ...
    sim_dir = "results/training_lengths/{}-{}-{}-{}/".format(today, NUM_TRIALS, TRAINING_LENGTH_RANGE[0], TRAINING_LENGTH_RANGE[-1])
